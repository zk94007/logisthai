...
##################################################
	...
	my $res = rpc_list_queue ($FH_DBG, $mandant_id, $kundennr, $queue_server);

	foreach my $ind (0 .. scalar @{$res} - 1) {
		my $parms_href  = @{$res}[$ind];
		print "QUEUE ($parms_href->{id}/$parms_href->{priority}): kundennr: $parms_href->{kundennr} timestamp: $parms_href->{timestamp} $parms_href->{buchdatum} $parms_href->{total_invoices} $parms_href->{buchsymbol} $parms_href->{periode}\n";
	}

	#print "QUEUE: ", Dumper(\$res);



	my $res = rpc_list_queue_running ($FH_DBG, $mandant_id, $kundennr, $queue_server);

	#print "RUNNING: ", Dumper(\$res);
	foreach my $ind (0 .. scalar @{$res} - 1) {
		my $parms_href  = @{$res}[$ind];
